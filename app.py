...
from forms import NewSecretDeleteGetForm, RegisterForm, UpdateForm, GetScopesForm, UpdateScopesForm
app = Flask(__name__)
...
def get_rule_data(access_scopes):
    rule_data = {
    ...
    return rule_data
def get_app_data(client_name,client_uri,redirect_uri):
    app_data = {
...
def create_rule(res,headers,access_scopes,created_policy):
    ...
    rule_data = get_rule_data(access_scopes)
...
def register():
    ...
    if request.method == 'POST' and form.validate():
        ...
        # print(access_scopes)
        app_data = get_app_data(client_name,client_uri,redirect_uri)
        
        res = requests.post(f"{client_registration}", json=app_data, headers=headers)
        # print(res.json())
...
def update_app():
    ...
    if request.method == 'POST' and form.validate():
        ...
        res = requests.put(f"{client_registration}/{client_id}", json=app_data, headers=headers)
        # print(res.json())
...
@app.route('/update-scopes', methods=['GET','POST'])
def update_scopes():
    form = UpdateScopesForm(request.form)
    ...
    if request.method == 'POST' and form.validate():
        ...
        for policy in policies.json():
            if client_id in policy['conditions']['clients']['include']:
                rules = requests.get(f"{policy['_links']['rules']['href']}", headers=headers)
                rule_url = rules.json()[0]['_links']['self']['href']
                rule_data = get_rule_data(access_scopes)
                update_rule = requests.put(rule_url, json=rule_data, headers=headers)
                print(update_rule.status_code)
                if rules.status_code == 200:
                #     scopes = rules.json()[0]['conditions']['scopes']['include']
                    flash(f'Scopes updated successfully', 'success')
                    return redirect(url_for('index'))
        flash('No scopes found for your client id', 'warning')
    return render_template('updateScopes.html', form=form) 
